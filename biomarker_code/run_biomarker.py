...
import pandas as pd
from biomarker_code.subtypePredictor import optimise_all_models, refit_all_models, test_all_models
...
def main(args):
    ...
    # Training without feature selection to ensure the consistency of feature lists and make SHAP values comparable
    # Motivated by https://github.com/shap/shap/issues/457
    discovery_X_array, discovery_y = load_omics(omics_discovery_path, train_endpoint_path, endpoint, selected_feature_lists=selected_feature_lists)
    selected_feature_lists = [omics_data.columns.to_list() for omics_data in discovery_X_array]
    discovery_X= pd.concat(discovery_X_array, axis=1)
    print(f"Shape of discovery data: {discovery_X.shape}")

    cat_feats = ['subgroup'] if 'subgroup' in discovery_X.columns else []
    con_feats = [col for col in discovery_X.columns if col not in cat_feats]

    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=int(seed))
    splits = []
    for (tr,ts) in cv.split(discovery_X, discovery_y):
        splits.append((tr,ts))

    trained_models = optimise_all_models(discovery_X, discovery_y, con_feats, cat_feats,splits)
    refit_models = refit_all_models(discovery_X, discovery_y, results=trained_models, splits=splits, omicsComb=dataset, save_dir=interim_path, random_state=int(seed))
    ...
    validation_X_array, validation_y = load_omics(omics_validation_path, test_endpoint_path, endpoint, selected_feature_lists=selected_feature_lists)
    validation_X= pd.concat(validation_X_array, axis=1)
    test_all_models(validation_X, validation_y, refit_models, con_feats=con_feats, cat_feats=cat_feats, save_dir=interim_path, omicsComb=dataset, random_state=str(seed))
...
if __name__ == '__main__':    
    ...
    # Data type
    parser.add_argument('--dataset', type=str, default=dataset,nargs='?', help="type of datasets")
