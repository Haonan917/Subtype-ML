...
def optimise_all_models(X, y,con_feats, cat_feats, splits):
    ...
    svc_result = optimise_svc_without_fs(X, y, con_feats, cat_feats,cv=splits_copy)
    rf_result = optimise_rf_without_fs(X, y,con_feats, cat_feats, cv=splits_copy)
    logres_result = optimise_logres_without_fs(X, y, con_feats, cat_feats, cv=splits_copy)
    
    averaged_models = EnsembleModels(models = (logres_result, rf_result, svc_result))
...
def model_refit(X, y, model, cv, algo, omicsComb, save_dir, random_state):
    cv_models, cv_aucs, cv_ypreds, cv_yreals, cv_tprs, total_ypreds, total_yreals = ([] for _ in range(7))
    ...
    with open(file_path, 'a') as f:
        if not file_exists:
            f.write('data_type,algo,seed,train.val,mean_auc,median_auc,std_auc\n')
        
        f.write('{},{},{},{},{},{},{}\n'.format(omicsComb,algo,random_state,1,mean_auc,median_auc,std_auc))
...
def test_all_models(X,y,results,con_feats, cat_feats,omicsComb,save_dir,random_state):
    test_result = {}
    for model in results.keys():
        test_result[model] = model_test(X,y,results[model][0],con_feats=con_feats, cat_feats=cat_feats,algo=model, omicsComb=omicsComb, save_dir=save_dir, random_state=random_state)
    return test_result

def model_test(X, y, model, con_feats, cat_feats, algo, omicsComb, save_dir, random_state):
    ...
    df_data.to_csv(save_dir / 'ypred_{}_{}.csv'.format(algo, omicsComb), index=False)
    ...
    analyze_feature_importance(X, y, model, con_feats, cat_feats, algo, omicsComb, save_dir, random_state)
...
def analyze_feature_importance(X, y, model, con_feats, cat_feats,algo, omicsComb, save_dir, random_state):
    if algo == 'avg':
        return
    
    selected_features = get_feature_names(model, con_feats, cat_feats)
    observations = model[:-1].transform(X)
    observations = observations.toarray() if hasattr(observations, "toarray") else observations
    ...
    elif algo in model.named_steps and isinstance(model.named_steps[algo], RandomForestClassifier):
        # 对于随机森林模型
...
def optimise_logres_without_fs(X, y, con_feats, cat_feats ,cv=5):
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', Pipeline(steps=[('imputer', SimpleImputer(strategy='median')),('scaler', StandardScaler())]), con_feats),
            ('cat', Pipeline(steps=[('imputer', SimpleImputer(strategy='most_frequent')),('encoder', OneHotEncoder(handle_unknown='ignore', sparse=False))]),cat_feats)
        ]
    )
    logres = LogisticRegression(random_state=1, penalty='elasticnet', solver='saga', max_iter=10000, n_jobs=-1, class_weight='balanced')
    pipe = Pipeline(steps=[('preprocessor', preprocessor), ('lr', logres)])
...
def optimise_svc_without_fs(X, y,con_feats, cat_feats ,cv=5):
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', Pipeline(steps=[('imputer', SimpleImputer(strategy='median')),('scaler', StandardScaler())]), con_feats),
            ('cat', Pipeline(steps=[('imputer', SimpleImputer(strategy='most_frequent')),('encoder', OneHotEncoder(handle_unknown='ignore', sparse=False))]),cat_feats)
...
def optimise_rf_without_fs(X, y,con_feats, cat_feats ,cv=5):
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', Pipeline(steps=[('imputer', SimpleImputer(strategy='median')),('scaler', StandardScaler())]), con_feats),
            ('cat', Pipeline(steps=[('imputer', SimpleImputer(strategy='most_frequent')),('encoder', OneHotEncoder(handle_unknown='ignore', sparse=False))]),cat_feats)
